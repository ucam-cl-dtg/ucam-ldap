...
public class LDAPProvider {
	...
	/**
	 * Gets a list of every use attribute 
	 * Takes 1 argument: crsid, 
	 * @return HashMap<String, ?> results
	 */
	public static HashMap<String, ?> fullUserQuery(String crsid) {
		...
		try {
			...
			String cN = a.get("cn").get().toString();
			String sN = a.get("sn").get().toString();
			String email = a.get("mail").get().toString();
			
			ArrayList<String> institutions = new ArrayList<String>();
			enumResults = a.get("ou").getAll();	
			while(enumResults.hasMore()){
				institutions.add(enumResults.next().toString());
			}
			
			ArrayList<String> status = new ArrayList<String>();
			enumResults = a.get("misAffiliation").getAll();	
			while(enumResults.hasMore()){
				status.add(enumResults.next().toString());
			}
			
			ArrayList<String> photos = new ArrayList<String>();
			enumResults = a.get("jpegPhoto").getAll();	
			while(enumResults.hasMore()){
				byte[] p = (byte[])enumResults.next();
				photos.add(new String(Base64.encodeBase64(p)));	
			}
			
			results.put("cname", cN);
			results.put("surname", sN);
